...
```js
equal(shaker(1, 'banana', true), '1 banana cocktails')
```

#### shaker can make multiple milkshakes.

```js
equal(shaker(1, 'banana', false), '1 banana milkshakes')
```
...
#### countArgs returns the right results
...
Declare an `giveBack` function an argument and return a new function that when called will return the argument of the `giveBack` function
