...
In JS, `Object` are the most basic data structure, they are a way to group values
together.
...
We separate them with a `:`, to simplify, we will only use valid identifiers
...
> it is not required for the last property.
...
We will provide a `human` variable of type object just like the one you did in the
previous exercise
...
The syntax is the same, if the property already exist, its value will be
changed !
...
Since numbers are not valid identifiers we can not use the `.` operator to access
a value in an array, but we can use the `[index]` square brackets to access a
value.
...
Has we can see, here in usingSpace, I have 2 spaces and a special character, but that still
works, handy !
...
It's the same old recipe, we access + use the `=` _(assign operator)_ to set
the value.
