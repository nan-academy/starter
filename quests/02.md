...
Let's try with an object `house1`.
Given what we know, the first instinct would be to do something
like this:
...
let house2 = house1
```
If we console.log() both house1 and house2 we will obtain exactly the same result:

```js
console.log(house1) //{ door: 'blue', rooms: 3 }
console.log(house2) //{ door: 'blue', rooms: 3 }
```
So far, so good.
Now, let's get into the issue.
Let's paint the door of `house1` in `red`.

```js
house1.door = 'red'
```
If we console.log() both house1 and house2 we will now obtain:
```js
console.log(house1) //{ door: 'red', rooms: 3}
console.log(house2) //{ door: 'red', rooms: 3}
```
But wait a second... we just wanted to paint the door of `house1`. What happened here? Why are both doors painted in red?

This where the difference between copying primitives and references comes into play.

```js
let primitive1 = 1

let storage = primitive1 //storing the value of primitive1
let storage2 = house1 //storing a reference to house1
```
There are ways to do those object copies properly.
Understanding the limitations of each of these
ways is very important.
Which is why this time, you will have to do a bit of research...

### Instructions

We will declare the same `house1` seen in the lesson above.

- Declare `house2` which will be a `shallow copy` of `house1`.
- Declare `house3` which will be a `deep copy` of `house2`.
...
#### house2 and house3 are both objects

```js
equal(typeof house2, 'object')
equal(typeof house3, 'object')
```

#### house2 is not a reference copy
...
if (house2.door == 'red') {
  throw Error('when painting house1.door in red, house2.door is painted red too. You did not do a shallow copy')
}

```

#### house2 is not a shallow copy
...
house1.rooms.bathrooms = 3

if (house2.rooms.bathrooms != 3) {
  throw Error('when adding 2 bathrooms to house1, house2 did not get 2 extra bathrooms as well, it looks like you did a deep copy instead of a shallow one...')
}
```

#### house3 is not a reference copy
...
if (house3.door == 'red') {
  throw Error('when painting house1.door in red, house3.door is painted red too, you did not do a deep copy')
}

```

#### house3 is a deep copy
...
if (house3.rooms.bathrooms != 1) {
  throw Error('when adding 2 bathrooms to house1, house3 didgot extra bathrooms as well, it looks like you did a shallow copy instead of a deep copy...')
}
```

## 🌟 frozen: more about mutability

In the previous lessons you have seen some examples of values
reassignement along the way.
...
house2.door = 'red'
```
First the object `house1` has been replaced by the string `destroyed`.

Second the door of `house2` has been repainted with the color
`red`.

You will now have to find a way to prevent this...
A way of "freezing" your objects if you will...
...
Level 4 Alert!
A very smart `virus19` has escaped the laboratory!
You have 5 mins to fight this virus on two fronts:

- First freeze the object `virus19` so that it cannot mutate!
- Then release the countermeasure by declaring `vaccine` which will be a secure copy of the object `antivirus`.
...
if (Object.isFrozen(virus19) != true) {
  throw Error('you did not freeze the virus19 properly. It has mutated!')
}
...
```js
let antivirus = {
  secretMolecule1: true,
  secretMolecule2: true
}

// Your code

if (vaccine.secretMolecule1 != true) {
  throw Error('you did not copy the antivirus')
}

if (vaccine.secretMolecule2 != true) {
  throw Error('you did not copy the antivirus')
}
```
...
if (Object.isFrozen(vaccine) != true) {
