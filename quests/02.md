...
Let's try with an object `house1`. Given what we know, the first instinct would
...
If we `console.log()` both house1 and house2 we will obtain exactly the same
output:
...
So far, so good. Now, let's get into the issue. Let's paint the door of `house1`
in `red`.
...
But wait a second... we just wanted to paint the door of `house1`. What happened
here? Why are both doors painted in red?

This where the difference between copying primitives and references comes into
play.
...
There are ways to do those object copies properly. Understanding the limitations
of each of these ways is very important. Which is why this time, you will have
...
if (house2.door == 'red') {
  throw Error(
    'when painting house1.door in red, house2.door is painted red too. You did not do a shallow copy',
  )
}
```
...
if (house2.rooms.bathrooms != 3) {
  throw Error(
    'when adding 2 bathrooms to house1, house2 did not get 2 extra bathrooms as well, it looks like you did a deep copy instead of a shallow one...',
  )
}
```
...
if (house3.door == 'red') {
  throw Error(
    'when painting house1.door in red, house3.door is painted red too, you did not do a deep copy',
  )
}
```
...
if (house3.rooms.bathrooms != 1) {
  throw Error(
    'when adding 2 bathrooms to house1, house3 didgot extra bathrooms as well, it looks like you did a shallow copy instead of a deep copy...',
  )
}
```
...
Second the door of `house2` has been repainted with the color `red`.
...
```js
let antivirus = { C8H10N4O: true, C12H22O11: true }
const virus19 = {
...
if (vaccine.C8H10N4O != true) {
  throw Error('you did not copy the antivirus')
}

if (vaccine.C12H22O11 != true) {
  throw Error('you did not copy the antivirus')
}
```
