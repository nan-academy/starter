...
- Log `'The thruth was spoken.'` if the value of the provided variable `truth`
  is truthy
...
equal(args[0]?.[0], 'The thruth was spoken.')
```
...
`You can benefit our special promotion`
...
equal(args[0]?.[0], 'You can benefit our special promotion')
```
...
if (user.role === 'admin' || user.role === 'moderator') {
  console.log(user.name, 'as access to moderation pages.')
}
```
...
`customer` as the means to buy this ticket. The `customer` may have enough
`cash` or a `voucher`

Check if the provided variable `customer` can afford the ticket:

- he has enough cash (`customer.cash` property)
- or, he has a voucher (`customer.hasVoucher` property is `true`)
...
You must define the `traveler.isImune` property to `true` if either:
...
equal(traveler.isImune, false)
```

#### Test a traveler that as recovered
...
equal(traveler.isImune, true)
```

#### Test a traveler never had the virus and isn't vaccinated
