...
To call for help you will have to concat the provided arrays `beacons1`,
`beacons2` and `beacons3` into the variable array `beaconsLit` that you will declare.
Only one array of burning fires must be left at the end.

Unfortunately young hobbit, the guards are watching the `Array.concat()` method.
...
```js
let beacons1 = ['Amon Dîn', 'Eilenach']
let beacons2 = ['Nardol', 'Erelas']
let beacons3 = ['Min-Rimmon', 'Calenhad', 'Amon Anwar']
Array.prototype.concat = undefined

// Your code

if (Array.isArray(beaconsLit) != true) {
...
if (beaconsLit.length != 7) {
  throw Error('All 7 beacons are not lit')
}
```
...
equal(beaconsLit.join(', '), 'Amon Dîn, Eilenach, Nardol, Erelas, Min-Rimmon, Calenhad, Amon Anwar')
```
...
The battle is won!
The Army of the Dead have finally fulfilled their oath.
It is now time to free them so that they may rest in peace.

But first we need to regroup and count them to make sure not
one single soul is left behind.
We will provide a variable of type array `armyOfTheDead`.
And a function called `regroup()`.
The function `regroup` can take as many arguments of type number as possible
and can return the sum of all those numbers.

The array `armyOfTheDead`, will vary in size from one test to another.

Find a way to use `regroup` with `armyOfTheDead`.

Declare the result in a variable called `freedSouls`
...
#### armyOfTheDead is of length 1

```js
let regroup = (...theSouls) => theSouls.reduce((prev, current)=> prev+current)
let armyOfTheDead = [1]
...
#### armyOfTheDead is of length 2

```js
let regroup = (...theSouls) => theSouls.reduce((prev, current)=> prev+current)
let armyOfTheDead = [1, 4]

// Your code

equal(freedSouls, 5)
```
#### armyOfTheDead is of length 3

```js
let regroup = (...theSouls) => theSouls.reduce((prev, current)=> prev+current)
let armyOfTheDead = [1, 4, 10]

// Your code

equal(freedSouls, 15)
```
#### armyOfTheDead is of length 4

```js
let regroup = (...theSouls) => theSouls.reduce((prev, current)=> prev+current)
let armyOfTheDead = [10, 4, 100, 1000]

// Your code

equal(freedSouls, 1114)
```
#### armyOfTheDead is of length 5

```js
let regroup = (...theSouls) => theSouls.reduce((prev, current)=> prev+current)
let armyOfTheDead = [10, 4, 100, 1000, 500]

// Your code

equal(freedSouls, 1614)
```
