...
> `var`'s since 2015. It's ancient syntax and it was pretty problematic. Never use
...
We will provide a variable `name` and `age`. 
They will be pre-declared by us.
...
Let's use string placeholders to duplicate a string, so that we can make sure we get
our point across.
...
a bit more by yourself and show you more obscure / weird aspects of the language.
