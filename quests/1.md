...
A string is a sequence of characters used to represent text, it need
**delimiters** to define its _begining_ and _end_.\
Delimitiers are matching quotes, either `` ` ``, `"` or `'`.
...
Lastly, a values, like the one you already know: `string`, `number` and
`boolean`.
...
Everytime their is an _extra special_ character into your string, putting a `\`
in front of it will **escape** it and doing so will let JS understand you ment
...
But it is still possible to use the `=` _(assignation operator)_ to change it's
value !
...
And the are resulting in a value, as such they can be assign to variables:
...
Let's use string placeholders to duplicate a string, so we can make sure we get
our point across.
