...
function assertNotStrictEquals1(actual, expected, msg) {
    ...
    throw new AssertionError1(msg ?? `Expected "actual" to be strictly unequal to: ${_format1(actual)}\n`);
}
...
function assertArrayIncludes1(actual, expected, msg) {
    ...
    if (!msg) {
        msg = `actual: "${_format1(actual)}" expected to include: "${_format1(expected)}"\nmissing: ${_format1(missing)}`;
    }
    throw new AssertionError1(msg);
}
...
function assertObjectMatch1(actual, expected) {
    ...
    return assertEquals1(function filter(a, b) {
...
async function assertThrowsAsync1(fn, ErrorClass, msgIncludes = "", msg) {
    let doesThrow = false;
    let error = null;
    try {
        await fn();
...
export { assertThrowsAsync1 as assertThrowsAsync };
